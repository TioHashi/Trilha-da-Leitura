...
function registrosMaisRecentesPorAluno(registrosBase = resultadosFiltrados || []) {
    const mapa = new Map();
...
function renderizarAnalise(resposta) {
    ...
    elemento("iaImprimir").disabled = false;
...
async function gerarAnalise() {
    ...
    elemento("iaImprimir").disabled = true;
    try {
...
function tituloRelatorioParaImpressao(relatorio) {
    const titulo = relatorio.querySelector(".report-cover h3")?.textContent
        || relatorio.querySelector("h3")?.textContent
...
function inicializar() {
    ...
    elemento("iaImprimir").addEventListener("click", imprimirRelatorio);
